...
def signal(direction="LONG", ts=BASE):
    return PullbackSignal(25, ts, direction, Decimal("100"), Decimal("110"), 0.4, Decimal("106"), Decimal("102"), 0.8, False, "EXPERIMENTAL_V2")
...
def test_trigger_freezes_snapshot_and_prevents_duplicate(tmp_path):
    ...
    assert state.snapshot.trigger_price == Decimal("106") and state.snapshot.target_1 == Decimal("110") and state.snapshot.target_2 == Decimal("114")
...
def test_target_one_hit_closes_and_emits_event(tmp_path):
    e = engine(tmp_path); state = e.trigger(signal(), candle()); events = e.update_tick(tick("110", BASE + timedelta(minutes=1)))
...
def test_target_two_can_close_directly_on_jump(tmp_path):
    e = engine(tmp_path); e.trigger(signal(), candle()); events = e.update_tick(tick("114", BASE + timedelta(minutes=1)))
    assert [x["event"] for x in events] == ["TARGET_1_REACHED", "SETUP_CLOSED"] and events[-1]["outcome"] == "TARGET_1_HIT"
...
def test_cooldown_blocks_rearm_then_allows_rearm(tmp_path):
    e = engine(tmp_path, cooldown_seconds=300); first = e.trigger(signal(), candle()); e.update_tick(tick("110", BASE + timedelta(minutes=1)))
...
def test_mfe_mae_are_tracked_from_live_prices(tmp_path):
    e = engine(tmp_path); e.trigger(signal(), candle()); e.update_tick(tick("109", BASE + timedelta(minutes=1))); e.update_tick(tick("104", BASE + timedelta(minutes=2)))
    assert e.active[25].mfe == Decimal("3") and e.active[25].mae == Decimal("2")


def test_lifecycle_replays_after_process_restart(tmp_path):
    e = engine(tmp_path, cooldown_seconds=300); e.trigger(signal(), candle()); e.update_tick(tick("110", BASE + timedelta(minutes=1))); restored = engine(tmp_path, cooldown_seconds=300)
