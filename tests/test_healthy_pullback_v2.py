...
def engine():
    ...
    e.history = [c(i, 100+i*0.1) for i in range(5)]; e._seen_candles = {x.start for x in e.history}
    ...
    e.candidate_id = "fixture"; e.candidate_created_at = e.history[-1].end
    return e


def stats(depth=.4, speed=.4, efficiency=.5, body_ratio=1.0, internal=1, volume=.9, overlap=.2):
...
def run(e, p):
    ...
    return e.update(c(7, 108.5, 109, 107.5))


def test_clean_bullish_impulse_healthy_pullback_reaches_trigger():
...
def test_weak_impulse_fixture_is_below_minimum():
    e=engine(); e.impulse.atr_multiple=.9; assert e.impulse.atr_multiple < e.cfg["min_impulse_atr"]


def test_noisy_impulse_fixture_has_low_efficiency():
    e=engine(); e.impulse.efficiency=.42; assert e.impulse.efficiency < e.cfg["min_impulse_efficiency"]


def test_excessive_retracement_rejected():
    e=engine(); p=stats(depth=.76); e._reject(BASE,"PULLBACK_HEALTH","EXCESSIVE_RETRACEMENT",p["depth"],.75); assert e.last_rejection["reason"] == "EXCESSIVE_RETRACEMENT"


def test_protected_swing_break_is_critical():
    e=engine(); e._reject(BASE,"STRUCTURE","PROTECTED_STRUCTURE_BROKEN",98,99); assert e.last_rejection["reason"] == "PROTECTED_STRUCTURE_BROKEN"


def test_aggressive_countertrend_acceleration_rejection():
    e=engine(); e._reject(BASE,"PULLBACK_HEALTH","COUNTERTREND_ACCELERATION",1.2,1.1); assert e.last_rejection["reason"] == "COUNTERTREND_ACCELERATION"


def test_high_pullback_efficiency_is_reversal_risk():
    e=engine(); e._reject(BASE,"PULLBACK_HEALTH","PULLBACK_REVERSAL_EVIDENCE",.9,.85); assert e.last_rejection["reason"] == "PULLBACK_REVERSAL_EVIDENCE"


def test_severe_chop_requires_multiple_signals():
    e=engine(); p=stats(internal=5, overlap=.5, efficiency=.5); assert p["internal_swings"] > e.cfg["max_internal_swings"] and p["overlap"] > .35 and p["efficiency"] < .70


def test_high_opposing_volume_combined_with_aggression():
    e=engine(); p=stats(volume=1.35, efficiency=.75); assert p["volume_ratio"] > e.cfg["volume_reject_ratio"] and p["efficiency"] > e.cfg["volume_efficiency_reject"]
...
def test_score_is_capped_at_100():
    e=engine(); p=stats(speed=.1, efficiency=.1, volume=.1); assert e._score(e.impulse,p,True) <= 100


def test_continuation_requires_displacement_beyond_trigger():
    e=engine(); p=stats(); e._score=lambda *args:90; signal=run(e,p); assert signal is not None and signal.classification == "TRIGGER_CONFIRMED"


def test_trigger_snapshot_has_immutable_metrics():
    e=engine(); p=stats(); e._score=lambda *args:90; s=run(e,p); assert s.signal_id == "fixture" and s.health_score == 90 and s.impulse_range == Decimal("11")
...
def test_session_boundary_after_1530_is_invalid():
    e=engine(); result=e.update(Candle(1,datetime(2026,8,11,10,30,tzinfo=timezone.utc),datetime(2026,8,11,10,35,tzinfo=timezone.utc),Decimal(100),Decimal(101),Decimal(99),Decimal(100),1000,True,300)); assert result is None and e.last_rejection["reason"] == "SESSION_INVALID"


def test_stale_future_data_is_not_accepted():
    e=engine(); future=datetime.now(timezone.utc)+timedelta(minutes=10); result=e.update(Candle(1,future-timedelta(minutes=5),future,Decimal(100),Decimal(101),Decimal(99),Decimal(100),1000,True,300)); assert result is None and e.last_rejection["reason"] == "DATA_INVALID"
