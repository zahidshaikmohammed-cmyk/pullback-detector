...
class EventStore:
    ...
    def _append(self,path:Path,record:dict)->None:
        path.parent.mkdir(parents=True,exist_ok=True);line=json.dumps(record,default=_jsonable,separators=(",", ":"))+"\n"
        with path.open("a",encoding="utf-8") as handle:handle.write(line);handle.flush();os.fsync(handle.fileno())
    ...
    def tick(self,received_at:datetime,tick:Tick)->None:
        day=received_at.astimezone(timezone.utc).strftime("%Y-%m-%d");record=asdict(tick);record["received_at"]=received_at;self._append(self.root/"normalized"/f"{day}.jsonl",record)

    def candle(self,candle:Candle)->None:
        day=candle.start.astimezone(timezone.utc).strftime("%Y-%m-%d");self._append(self.root/"candles"/f"{day}.jsonl",asdict(candle))

    def signal(self,signal:PullbackSignal)->None:
        day=signal.timestamp.astimezone(timezone.utc).strftime("%Y-%m-%d");self._append(self.root/"signals"/f"{day}.jsonl",asdict(signal))

    def health(self,report:dict)->None:self._append(self.root/"health.jsonl",report)
