...
class EventStore:
    ...
    def __init__(self, root: str | Path = "data/runtime"):
        ...
        self._candle_keys = set(); self._event_keys = set(); self._signal_keys = set()
        self._persisted_counts = {60: 0, 300: 0}; self._last_persisted_timestamp = {60: None, 300: None}
        self.persistence_write_count = 0; self.persistence_failure_count = 0
        self.duplicate_event_count = 0; self.duplicate_signal_count = 0; self.duplicate_candle_contribution_count = 0
        self._current_run_started_at = datetime.now(timezone.utc); self._first_tick_after_start = None; self._first_candle_after_start = None
        self._recovery_probe_passed = False; self._previous_checkpoint = self._load_recovery_checkpoint(); self._previous_health_report = self._load_last_health_report()
        self._load_indexes(); self._run_recovery_identity_probe(); EventStore._last_instance = self

    def _append(self, path: Path, record: dict) -> None:
        try:
            path.parent.mkdir(parents=True, exist_ok=True)
            with path.open("a", encoding="utf-8") as handle:
                handle.write(json.dumps(record, default=_jsonable, separators=(",", ":")) + "\n"); handle.flush(); os.fsync(handle.fileno())
            try: os.chmod(path, 0o600)
            except OSError: pass
        except Exception:
            self.persistence_failure_count += 1; raise
    ...
    def _event_key_from_row(row: dict) -> str | None:
        try:
            ...
            raw = "|".join((str(row.get("exchange_segment", "NSE_EQ")), str(row["instrument_id"]), ts, str(row["price"]), str(row.get("quantity", 0)), str(row.get("cumulative_volume")), str(row.get("sequence"))))
            return sha256(raw.encode("utf-8")).hexdigest()
            ...
            for file in sorted(candles_path.glob("*.jsonl")):
                try:
                    for line in file.read_text(encoding="utf-8").splitlines():
                        ...
                        self._candle_keys.add(key); timeframe = key[1]
                        if timeframe in self._persisted_counts:
                            self._persisted_counts[timeframe] += 1; self._last_persisted_timestamp[timeframe] = max(self._last_persisted_timestamp[timeframe] or key[2], key[2])
    ...
    def _run_recovery_identity_probe(self):
        """Verify persisted identity rejects the same event and accepts a genuinely new event."""
                ...
                try:
                    for line in reversed(file.read_text(encoding="utf-8").splitlines()):
                        try:
                            ...
                            if key:
                                sample = (row, key); break
                        except json.JSONDecodeError: continue
                except OSError: continue
                if sample: break
        if not sample:
            self._recovery_probe_passed = not self._event_keys
            return
        row, key = sample
        same_event_rejected = key in self._event_keys
        # A deterministic new identity must differ without relying on receive/process identity.
        new_row = dict(row); new_row["sequence"] = (int(row.get("sequence") or 0) + 1)
        new_key = self._event_key_from_row(new_row)
        self._recovery_probe_passed = bool(same_event_rejected and new_key and new_key != key)
    ...
    def raw_packet(self, received_at, payload, response_code=None):
        day = received_at.astimezone(timezone.utc).strftime("%Y-%m-%d"); self._append(self.root / "raw" / f"{day}.jsonl", {"received_at": received_at, "response_code": response_code, "payload_hex": payload.hex()})

    def tick(self, received_at, tick: Tick) -> bool:
        key = self._event_key(tick)
        if key in self._event_keys:
            self.duplicate_event_count += 1; return False
        self._event_keys.add(key)
        if self._first_tick_after_start is None: self._first_tick_after_start = datetime.now(timezone.utc)
        day = received_at.astimezone(timezone.utc).strftime("%Y-%m-%d"); record = asdict(tick); record["received_at"] = received_at
        self._append(self.root / "normalized" / f"{day}.jsonl", record); self.persistence_write_count += 1; return True
    ...
    def candle(self, candle: Candle) -> bool:
        ...
        if key in self._candle_keys:
            self.duplicate_candle_contribution_count += 1; return False
        self._candle_keys.add(key); timeframe = candle.timeframe_seconds
        if timeframe in self._persisted_counts:
            self._persisted_counts[timeframe] += 1; value = candle.start.astimezone(timezone.utc).isoformat(); self._last_persisted_timestamp[timeframe] = max(self._last_persisted_timestamp[timeframe] or value, value)
        if self._first_candle_after_start is None: self._first_candle_after_start = datetime.now(timezone.utc)
        day = candle.start.astimezone(timezone.utc).strftime("%Y-%m-%d"); self._append(self.root / "candles" / f"{day}.jsonl", asdict(candle)); self.persistence_write_count += 1; return True
    ...
    def signal(self, signal: PullbackSignal) -> bool:
        ...
        self._signal_keys.add(key); day = signal.timestamp.astimezone(timezone.utc).strftime("%Y-%m-%d"); self._append(self.root / "signals" / f"{day}.jsonl", asdict(signal)); self.persistence_write_count += 1; return True
    ...
    def recovery_snapshot(self):
        previous = self._previous_checkpoint or {}; pre_1m = int(previous.get("persisted_1m_candles", 0) or 0); pre_5m = int(previous.get("persisted_5m_candles", 0) or 0)
        post_1m = self._persisted_counts[60]; post_5m = self._persisted_counts[300]; recovered = pre_1m > 0 or pre_5m > 0
        history_restored = post_1m >= pre_1m and post_5m >= pre_5m
        continued_events = self._first_tick_after_start is not None; continued_candles = self._first_candle_after_start is not None
        continuity = history_restored and self._recovery_probe_passed and self.duplicate_candle_contribution_count == 0 and self.duplicate_signal_count == 0
        verified = bool(recovered and history_restored and self._recovery_probe_passed and continued_events and continued_candles and self.persistence_failure_count == 0 and self.duplicate_candle_contribution_count == 0)
        recovery_time = self._first_candle_after_start or self._first_tick_after_start; duration_ms = max(0.0, (recovery_time - self._current_run_started_at).total_seconds() * 1000.0) if recovery_time else None
    ...
    def counter_progression(self, current_health):
        previous = self._previous_health_report or {}; keys = ("accepted_tick_count", "ticks_sent_to_candle_engine", "completed_1m_candles", "completed_5m_candles", "persisted_candle_count_1m", "persisted_candle_count_5m")
        before = {k: int(previous.get("cumulative_counter_values", {}).get(k, previous.get(k, 0)) or 0) for k in keys}; current = {k: int(current_health.get(k, 0) or 0) for k in keys}
        # The stored health report contains cumulative values; compare current cumulative counters directly.
        comparable = bool(previous); after = current; verified = comparable and all(after[k] >= before[k] for k in keys) and any(after[k] > before[k] for k in keys)
