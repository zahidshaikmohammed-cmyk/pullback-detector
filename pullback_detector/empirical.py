...
def evaluate_signals(signals: Iterable[PullbackSignal], candles: pd.DataFrame, max_horizon_bars=60) -> SignalOutcomeMetrics:
    ...
    for signal in signals:
        ...
        if r == 0: continue
        mfe=Decimal("0"); mae=Decimal("0"); t1_hit=False; t2_hit=False; invalidated=False; outcome=None; close=signal.trigger_price
    ...
    return SignalOutcomeMetrics(len(signals),target1,target2,invalid,continuation,avg_r,avg_r,sum(mfes)/len(mfes) if mfes else 0.0,sum(maes)/len(maes) if maes else 0.0,sum(t1)/len(t1) if t1 else None,sum(invtime)/len(invtime) if invtimes else None)
