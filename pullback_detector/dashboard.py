"""Lightweight browser dashboard backed by the live detector state and runtime store."""
...
class DashboardData:
    """Read-only projection of accepted market data and the actual V1 detector state."""
    ...
    def snapshot(self):
        ...
        cache_key = (minute_file.stat().st_mtime if minute_file.exists() else 0.0, str(self.state.get("status")), str(self.state.get("last_error") or ""), len(LIVE_ANATOMY))
        with self._lock:
            ...
            active = [s for s in signals if self._parse_time(s.get("timestamp")) and self._parse_time(s["timestamp"]) >= cutoff]
            ...
            payload = {"generated_at": now.isoformat(), "health": health, "instruments": instruments, "active_signals": active[:25], "recent_signals": signals[:50], "signal_window_minutes": 30}
