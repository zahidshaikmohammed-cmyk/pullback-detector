...
def benchmark_alignment(stock: Mapping, benchmarks: Mapping[str, Mapping]) -> dict:
    ...
    if not comparisons:
        return {"status":"INSUFFICIENT_DATA","score":None,"matching_timeframes":None,"compared_timeframes":0,"nifty":benchmark_views["NIFTY"],"banknifty":benchmark_views["BANKNIFTY"]}
    ratio = sum(comparisons) / len(comparisons)
    status = "ALIGNED" if ratio == 1.0 else "PARTIALLY_ALIGNED" if ratio >= 0.5 else "DIVERGING"
    return {"status":status,"score":int(round(ratio*100)),"matching_timeframes":sum(comparisons),"compared_timeframes":len(comparisons),"nifty":benchmark_views["NIFTY"],"banknifty":benchmark_views["BANKNIFTY"]}
...
class MarketContextEngine:
    ...
    def update_tick(self, tick: Tick) -> None:
        ...
        try:
            ...
            self.last_context = {"instrument_id":self.instrument_id,"data_freshness":"CALCULATION_ERROR","context_error":self.last_error}
            logger.exception("market context calculation failed instrument_id=%s", self.instrument_id)
    ...
    def update_candle(self, candle: Candle) -> None:
        ...
        try:
            ...
            self.last_error = f"{type(exc).__name__}: {exc}"
            self.last_context = {"instrument_id":self.instrument_id,"data_freshness":"CALCULATION_ERROR","context_error":self.last_error}
            logger.exception("market context candle calculation failed instrument_id=%s", self.instrument_id)

    @staticmethod
    def _ema(values: list[float], period: int) -> float | None:
        if len(values) < period:return None
        k=2.0/(period+1); value=sum(values[:period])/period
    ...
    def _efficiency(bars: list[Candle]) -> float | None:
        ...
        net=abs(float(bars[-1].close-bars[0].open)); travel=sum(abs(float(b.close-b.open)) for b in bars)
        return min(1.0,net/travel) if travel else 0.0

    @staticmethod
    def _roc(bars: list[Candle], n: int) -> float | None:
        if len(bars)<=n:return None
        base=float(bars[-n-1].close); return (float(bars[-1].close)-base)/base if base else None
    ...
    def _atr_percentile(bars: list[Candle], period: int = 14) -> float | None:
        ...
        for i in range(period-1,len(bars)):
            value=MarketContextEngine._atr(bars[:i+1],period)
    ...
    def _trend(self,bars:list[Candle],strong_threshold:float=0.004)->dict:
        if len(bars)<20:
            return {"direction":"INSUFFICIENT_DATA","score":None,"strength":None,"momentum":"INSUFFICIENT_DATA","structure":"INSUFFICIENT_DATA","volatility":"INSUFFICIENT_DATA","efficiency":None,"protected_level":None,"reference_level":None,"ema20":None,"ema50":None,"atr":None,"roc":None,"atr_percentile":None}
        closes=[float(b.close) for b in bars]; e20=self._ema(closes,20); e50=self._ema(closes,50); atr=self._atr(bars); structure,protected,reference,_=self._swing_structure(bars); roc=self._roc(bars,min(10,max(2,len(bars)//5))) or 0; slope=self._slope(closes,5) or 0; eff=self._efficiency(bars[-20:]) or 0
        bull=0; bear=0
        if e20 is not None and e50 is not None: bull+=20 if e20>e50 else 0; bear+=20 if e20<e50 else 0
        if slope>0:bull+=15
        ...
        score=max(bull,bear); direction="BULLISH" if bull>bear+10 else "BEARISH" if bear>bull+10 else "NEUTRAL"; label="STRONG "+direction if score>=75 else direction if score>=55 else "NEUTRAL"
        mom="STRENGTHENING" if (slope*roc)>0 and abs(roc)>0 else "WEAKENING" if (slope*roc)<0 else "STABLE"; vol_pct=self._atr_percentile(bars); vol="INSUFFICIENT_DATA" if vol_pct is None else "VERY LOW" if vol_pct<10 else "LOW" if vol_pct<30 else "NORMAL" if vol_pct<70 else "HIGH" if vol_pct<90 else "EXTREME"
    ...
    def _vwap(self,bars:list[Candle])->dict:
        base={"state":"INSUFFICIENT_DATA","distance_atr":None,"slope":None,"crosses":None,"time_above":None,"time_below":None,"vwap":None}
        if not bars:return base
        local_day=bars[-1].start.astimezone(self.tz).date(); day=[b for b in bars if b.start.astimezone(self.tz).date()==local_day]
        if not day:return base
        den=sum(b.volume for b in day); vwap=float(sum(((b.high+b.low+b.close)/Decimal(3))*b.volume for b in day)/den) if den else None
        if vwap is None:return base
        atr=self._atr(bars); price=float(self.last_tick.price) if self.last_tick else float(day[-1].close); crosses=sum((float(a.close)-vwap)*(float(b.close)-vwap)<0 for a,b in zip(day[-10:-1],day[-9:])); above=sum(float(b.close)>=vwap for b in day); below=len(day)-above; slope=(vwap-float(((day[-2].high+day[-2].low+day[-2].close)/Decimal(3)))) if len(day)>1 else 0
    ...
    def _volume(bars:list[Candle])->dict:
        ...
        vals=[b.volume for b in bars]; med=float(median(vals[-20:])); cur=float(vals[-1]); rv=cur/med if med else None; state="VERY LOW" if rv is not None and rv<.5 else "LOW" if rv is not None and rv<.8 else "NORMAL" if rv is not None and rv<1.2 else "ELEVATED" if rv is not None and rv<1.5 else "HIGH" if rv is not None and rv<2 else "EXTREME"; trend="RISING" if sum(vals[-5:])/5>sum(vals[-10:-5])/5 else "FALLING" if sum(vals[-5:])/5<sum(vals[-10:-5])/5 else "STABLE"
    ...
    def _chop(self,bars:list[Candle])->dict:
        ...
        eff=self._efficiency(bars[-20:]) or 0; directions=[b.close>b.open for b in bars[-20:]]; alt=sum(a!=b for a,b in zip(directions[:-1],directions[1:]))/max(1,len(directions)-1); overlap=sum(max(0.0,min(float(a.high),float(b.high))-max(float(a.low),float(b.low))) for a,b in zip(bars[-20:-1],bars[-19:]))/max(1,len(bars)-1); rng=[float(b.high-b.low) for b in bars[-20:]]; compression=(median(rng)/max(rng)) if rng else 0; crosses=self._vwap(bars).get("crosses") or 0; typical_range=median(rng) if rng else 0; overlap_ratio=(overlap/typical_range) if typical_range else 0; score=max(0,min(100,round((1-eff)*45+alt*25+min(1,overlap_ratio)*15+min(1,crosses/5)*10+(1-compression)*5))); state="CLEAN" if score<25 else "MILD CHOP" if score<45 else "CHOPPY" if score<70 else "SEVERE CHOP"
    ...
    def _rebuild(self)->None:
        ...
        dirs=[_base_direction(trends[x]["direction"]) for x in ("day","h1","m15","m5") if _base_direction(trends[x]["direction"]) in ("BULLISH","BEARISH")]; bulls=sum(x=="BULLISH" for x in dirs); bears=sum(x=="BEARISH" for x in dirs); alignment=f"{bulls}/{len(dirs)} BULLISH" if bulls>=bears and dirs else f"{bears}/{len(dirs)} BEARISH" if bears>bulls else "CONFLICTED"
        structure=trends["m5"].get("structure","INSUFFICIENT_DATA"); primary=trends["current"]["direction"]; health=None; evidence=[]; conflicts=[]
        if trends["m5"].get("score") is not None:evidence.append({"factor":"5M TREND","score":trends["m5"]["score"]})
        ...
        chop_score=chop.get("score")
        if chop_score is not None:conflicts.append({"factor":"CHOP","score":chop_score})
        if evidence:health=int(round(sum(x["score"] for x in evidence)/len(evidence)-sum(x["score"]*.15 for x in conflicts)/max(1,len(conflicts))))
    ...
    def _minutes_since_open(self,ts):
        local=ts.astimezone(self.tz); open_dt=local.replace(hour=9,minute=15,second=0,microsecond=0); return max(0,int((local-open_dt).total_seconds()/60)) if local>=open_dt else 0

    def _minutes_to_close(self,ts):
        local=ts.astimezone(self.tz); close_dt=local.replace(hour=15,minute=30,second=0,microsecond=0); return max(0,int((close_dt-local).total_seconds()/60)) if local<=close_dt else 0
