...
def _safe_store(health, operation, *args):
    try:
        return getattr(operation[0], operation[1])(*args)
    except Exception as exc:
        health.persistence_errors += 1
        logger.exception("PERSISTENCE_ERROR operation=%s error=%s", operation[1], exc)
        return None
...
def _equity_context(contexts, benchmark_contexts, instrument_id: int) -> dict:
    ...
    stock.update({
        ...
        "stock_vs_market_alignment": alignment["status"],
        "stock_vs_market_alignment_score": alignment["score"],
        "stock_vs_market_compared_timeframes": alignment["compared_timeframes"],
        "market_alignment": alignment["status"],
    })
    return stock


def _publish_anatomy(detectors, contexts, benchmark_contexts, instrument_id: int, data_root=None) -> None:
    ...
    try:
        ...
        logger.exception("anatomy publication failed instrument_id=%s", instrument_id)
        LIVE_ANATOMY[instrument_id] = {
            "instrument_id": instrument_id,
            "data_freshness": "CALCULATION_ERROR",
...
def _safe_context_tick(context, tick):
    try:
        ...
        logger.exception("context tick update failed instrument_id=%s", context.instrument_id)
...
def _safe_context_candle(context, candle):
    try:
        ...
        logger.exception("context candle update failed instrument_id=%s", context.instrument_id)
...
def _emit_v2_signal(detectors, contexts, benchmark_contexts, candle, store, lifecycle, data_root, health):
    ...
    try:
        ...
        _publish_anatomy(detectors, contexts, benchmark_contexts, candle.instrument_id, data_root)
        if signal is None:
            return
        ...
        logger.exception("V2 detector cycle failed instrument_id=%s; continuing other instruments", candle.instrument_id)
...
async def run_live(settings: Settings, duration_seconds: int = 600) -> dict:
    ...
    benchmarks = InstrumentUniverse.fetch_benchmarks()
    expected = instruments + benchmarks
    store = EventStore(settings.data_root)
    InstrumentUniverse.write_snapshot(instruments, Path(settings.data_root) / "universe.csv")
    if benchmarks:
        InstrumentUniverse.write_snapshot(benchmarks, Path(settings.data_root) / "benchmarks.csv")
    ...
    try:
        ...
        logger.warning("NSE_EQ Market Quote verification unavailable; live WebSocket remains authoritative: %s", exc)
    if benchmarks:
        logger.info("benchmark LTP REST verification skipped; Dhan WebSocket is authoritative for NIFTY/BANKNIFTY and 429 cannot break live feed")
    ...
    registry = {i.security_id: i for i in expected}
    ...
    LIVE_RUNTIME.update({
        "health": health,
        "expected_instruments": [
            {"symbol": i.symbol, "security_id": i.security_id, "exchange_segment": i.exchange_segment, "instrument_type": i.instrument_type}
            for i in expected
        ],
        "persisted_1m": 0,
        "persisted_5m": 0,
    ...
    for instrument in instruments:
        c1 = store.recent_candles(instrument.security_id, 60, 2500)
        c5 = store.recent_candles(instrument.security_id, 300, 2500)
        persisted_1m += len(c1)
        persisted_5m += len(c5)
        for candle in c1:
            _safe_context_candle(contexts[instrument.security_id], candle)
        for candle in c5:
            _safe_context_candle(contexts[instrument.security_id], candle)
            detectors[instrument.security_id].history.append(candle)
            detectors[instrument.security_id]._seen_candles.add(candle.start)
        _publish_anatomy(detectors, contexts, benchmark_contexts, instrument.security_id, settings.data_root)
    for benchmark in benchmarks:
        for candle in store.recent_candles(benchmark.security_id, 60, 2500):
            _safe_context_candle(benchmark_contexts[benchmark.symbol], candle)
        for candle in store.recent_candles(benchmark.security_id, 300, 2500):
            _safe_context_candle(benchmark_contexts[benchmark.symbol], candle)

    LIVE_RUNTIME["persisted_1m"] = persisted_1m
    LIVE_RUNTIME["persisted_5m"] = persisted_5m
    LIVE_RUNTIME["restart_recovery_verified"] = bool(persisted_1m or persisted_5m)

    logger.info("FEED_CONNECTED expected_subscriptions=%d", len(subscriptions))
    deadline = asyncio.get_running_loop().time() + duration_seconds
    try:
        async for payload, tick, received_at in client.stream(subscriptions, request_code=17):
            LIVE_RUNTIME["websocket_connected"] = True
            ...
            if instrument is None:
                health.malformed_packets += 1
                logger.warning("TICK_REJECTED reason=UNKNOWN_INSTRUMENT instrument_id=%s", tick.instrument_id)
                continue
            ...
            normalized_tick, _ = normalize_live_tick_clock(tick, received_at, settings.max_future_seconds)
            result = validate_tick_detailed(normalized_tick, received_at, settings.max_future_seconds, settings.max_tick_age_seconds)
            if not result.valid:
                health.malformed_packets += 1
                logger.warning(
                    "TICK_REJECTED reason=%s instrument_id=%s actual=%s threshold=%s source_ts=%s receive_ts=%s",
                    result.reason_code,
                    tick.instrument_id,
                    result.actual_value,
                    result.threshold,
            ...
            tick = replace(normalized_tick, validation_status="VALIDATED")
            health.record_tick(tick, received_at)
            _safe_store(health, (store, "tick"), received_at, tick)
            lifecycle.update_tick(tick)

            context = contexts.get(tick.instrument_id)
            benchmark_context = next((ctx for ctx in benchmark_contexts.values() if ctx.instrument_id == tick.instrument_id), None)
            if context:
                _safe_context_tick(context, tick)
            if benchmark_context:
                _safe_context_tick(benchmark_context, tick)
            ...
            try:
                ...
                logger.exception("candle engine rejected accepted tick instrument_id=%s", tick.instrument_id)
                continue

            for candle in one_min.flush(tick.timestamp):
                _safe_store(health, (store, "candle"), candle)
                health.candles_1m += 1
                health.record_candle(candle)
                context = contexts.get(candle.instrument_id)
                benchmark_context = next((ctx for ctx in benchmark_contexts.values() if ctx.instrument_id == candle.instrument_id), None)
                if context:
                    _safe_context_candle(context, candle)
                    _publish_anatomy(detectors, contexts, benchmark_contexts, candle.instrument_id, settings.data_root)
                if benchmark_context:
                    _safe_context_candle(benchmark_context, candle)
            ...
            for candle in five_min.flush(tick.timestamp):
                ...
                if context:
                    _emit_v2_signal(detectors, contexts, benchmark_contexts, candle, store, lifecycle, settings.data_root, health)

    except Exception:
        LIVE_RUNTIME["websocket_connected"] = False
        raise
    ...
    report = health.report(
        ...
        persisted_1m,
        persisted_5m,
        LIVE_RUNTIME["expected_instruments"],
        LIVE_RUNTIME["websocket_connected"],
        LIVE_RUNTIME["restart_recovery_verified"],
    )
    report.update({
        "raw_packet_count": health.packets + health.duplicate_packets,
        "decoded_packet_count": health.packets,
        "aggregator_1m": one_min.state_snapshot(),
        "aggregator_5m": five_min.state_snapshot(),
        "active_1m_buckets": one_min.state_snapshot()["open_bars"],
        "active_5m_buckets": five_min.state_snapshot()["open_bars"],
        "completed_1m_candles": health.candles_1m,
        "completed_5m_candles": health.candles_5m,
        "benchmark_instruments": [
            {"symbol": b.symbol, "security_id": b.security_id, "exchange_segment": b.exchange_segment}
            for b in benchmarks
        ],
        "benchmark_count": len(benchmarks),
        "alerts_enabled": False,
        "active_setup_count": len(lifecycle.active),
        "closed_setup_count": len(lifecycle.closed),
        "configured_instruments": len(subscriptions),
        "resolved_instruments": len(expected),
        "subscribed_instruments": len(subscriptions),
        "producing_instruments": len(health.instruments_seen),
        "market_context_engine": "deterministic_v1",
        "v2_detector": PullbackDetector.LABEL,
    })
    market_open = _nse_cash_session_open(datetime.now(timezone.utc))
    report.update({"market_status": "OPEN" if market_open else "CLOSED", "session_state": "NSE_CASH_SESSION_OPEN" if market_open else "NSE_CASH_SESSION_CLOSED"})
    if not market_open and health.ticks == 0:
        report.update({"dhan_connection_status": "session_closed", "no_live_data": True, "last_known_data_retained": True})
    ...
    if health.ticks > 0 and equity_seen < settings.min_live_instruments:
        raise RuntimeError(f"LIVE_VALIDATION_FAILED: only {equity_seen} equity instruments produced packets; minimum={settings.min_live_instruments}")
