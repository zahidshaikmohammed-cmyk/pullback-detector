...
LIVE_RUNTIME: dict = {"health": None, "expected_instruments": [], "persisted_1m": 0, "persisted_5m": 0, "websocket_connected": False, "restart_recovery_verified": False, "contexts": {}, "benchmark_contexts": {}, "detectors": {}, "one_min": None, "five_min": None}


def _safe_store(health, operation, *args):
    try: return getattr(operation[0], operation[1])(*args)
    except Exception as exc:
        health.persistence_errors += 1; logger.exception("PERSISTENCE_ERROR operation=%s error=%s", operation[1], exc); return None
...
def _equity_context(contexts, benchmark_contexts, instrument_id: int) -> dict:
    ...
    stock.update({"benchmark_context": benchmarks, "stock_vs_market_alignment": alignment["status"], "stock_vs_market_alignment_score": alignment["score"], "stock_vs_market_compared_timeframes": alignment["compared_timeframes"], "market_alignment": alignment["status"]}); return stock
...
async def run_live(settings: Settings, duration_seconds: int = 600) -> dict:
    ...
    try:
        async for payload, tick, received_at in client.stream(subscriptions, request_code=17):
            ...
            try: one_min.update(tick); five_min.update(tick)
            except Exception: health.record_candle_engine_reject(); logger.exception("candle engine rejected accepted tick instrument_id=%s", tick.instrument_id); continue
    ...
    market_open = _nse_cash_session_open(datetime.now(timezone.utc)); report.update({"market_status": "OPEN" if market_open else "CLOSED", "session_state": "NSE_CASH_SESSION_OPEN" if market_open else "NSE_CASH_SESSION_CLOSED"})
    ...
    equity_seen = sum(1 for segment, instrument_id in health.instruments_seen if segment == "NSE_EQ" and instrument_id in detectors)
