...
def _session_metrics(context:MarketContextEngine,ts:datetime)->dict:
    bars=context.candles_1m or context.candles_5m;local_day=ts.astimezone(context.tz).date();day=[b for b in bars if b.start.astimezone(context.tz).date()==local_day]
...
def _equity_context(contexts,benchmark_contexts,instrument_id:int)->dict:
    context=contexts.get(instrument_id);stock=dict(context.snapshot() if context else {});benchmarks={name:ctx.snapshot() for name,ctx in benchmark_contexts.items()};alignment=benchmark_alignment(stock,benchmarks)
    stock["benchmark_context"]=benchmarks;stock["stock_vs_market_alignment"]=alignment["status"];stock["stock_vs_market_alignment_score"]=alignment["score"];stock["stock_vs_market_compared_timeframes"]=alignment["compared_timeframes"];stock["market_alignment"]=alignment["status"]
    return stock
...
def _publish_anatomy(detectors,contexts,benchmark_contexts,instrument_id:int,data_root:str|Path|None=None)->None:
    ...
    try:
        anatomy=_equity_context(contexts,benchmark_contexts,instrument_id);anatomy.update(_session_metrics(context,datetime.now(timezone.utc)) if context else {});anatomy.update(detector.last_state or detector.anatomy());anatomy["market_context"]=_equity_context(contexts,benchmark_contexts,instrument_id);anatomy["market_context_version"]="1.1";LIVE_ANATOMY[instrument_id]=anatomy
...
def _safe_context_tick(context:MarketContextEngine,tick)->None:
...
def _safe_context_candle(context:MarketContextEngine,candle)->None:
...
async def run_live(settings:Settings,duration_seconds:int=600)->dict:
    ...
    detectors={i.security_id:PullbackDetector(instrument_id=i.security_id,audit_root=settings.data_root) for i in instruments};contexts={i.security_id:MarketContextEngine(i.security_id) for i in instruments};benchmark_contexts={i.symbol:MarketContextEngine(i.security_id) for i in benchmarks};LIVE_ANATOMY.clear()
    ...
    async for payload,tick,received_at in client.stream(subscriptions,request_code=17):
        ...
        context=contexts.get(tick.instrument_id);benchmark_for_tick=next((ctx for ctx in benchmark_contexts.values() if ctx.instrument_id==tick.instrument_id),None)
        if context:_safe_context_tick(context,tick)
        if benchmark_for_tick:_safe_context_tick(benchmark_for_tick,tick)
        for candle in one_min.flush(tick.timestamp):
            ...
            if benchmark_for_candle:_safe_context_candle(benchmark_for_candle,candle)
