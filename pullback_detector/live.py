...
def _persist_anatomy(data_root: str | Path, instrument_id: int, anatomy: dict) -> None:
    root = Path(data_root) / "anatomy"; root.mkdir(parents=True, exist_ok=True)
    target = root / f"{instrument_id}.json"; tmp = target.with_suffix(".tmp")
...
def _equity_context(contexts, benchmark_contexts, instrument_id: int) -> dict:
    context = contexts.get(instrument_id); stock = dict(context.snapshot() if context else {})
    benchmarks = {name: ctx.snapshot() for name, ctx in benchmark_contexts.items()}; alignment = benchmark_alignment(stock, benchmarks)
    stock.update({"benchmark_context": benchmarks, "stock_vs_market_alignment": alignment["status"], "stock_vs_market_alignment_score": alignment["score"], "stock_vs_market_compared_timeframes": alignment["compared_timeframes"], "market_alignment": alignment["status"]})
    return stock
...
def _publish_anatomy(detectors, contexts, benchmark_contexts, instrument_id: int, data_root: str | Path | None = None) -> None:
    ...
    try:
        ...
        logger.exception("anatomy publication failed instrument_id=%s", instrument_id); LIVE_ANATOMY[instrument_id] = {"instrument_id": instrument_id, "data_freshness": "CALCULATION_ERROR", "context_error": f"{type(exc).__name__}: {exc}"}
...
async def run_live(settings: Settings, duration_seconds: int = 600) -> dict:
    ...
    client = DhanWebSocketClient(settings.dhan_client_id, settings.dhan_access_token, settings.dhan_ws_url, settings.max_reconnects); health = ConnectivityHealth(); dedupe = PacketDeduplicator(settings.dedupe_capacity); one_min = CandleAggregator(60); five_min = CandleAggregator(300)
    ...
    registry = {i.security_id: i for i in instruments + benchmarks}; detectors = {i.security_id: PullbackDetector(instrument_id=i.security_id, audit_root=settings.data_root) for i in instruments}; contexts = {i.security_id: MarketContextEngine(i.security_id) for i in instruments}; benchmark_contexts = {i.symbol: MarketContextEngine(i.security_id) for i in benchmarks}; LIVE_ANATOMY.clear()
    for instrument in instruments:
        for candle in store.recent_candles(instrument.security_id, 60, 2500): _safe_context_candle(contexts[instrument.security_id], candle)
        for candle in store.recent_candles(instrument.security_id, 300, 2500): _safe_context_candle(contexts[instrument.security_id], candle); detectors[instrument.security_id].history.append(candle); detectors[instrument.security_id]._seen_candles.add(candle.start)
    ...
    async for payload, tick, received_at in client.stream(subscriptions, request_code=17):
        ...
        store.raw_packet(received_at, payload, payload[0] if payload else None)
        ...
        tick = replace(tick, symbol=instrument.symbol, instrument_type=instrument.instrument_type, source="DHAN", validation_status="RAW")
        ...
        tick = replace(normalized_tick, validation_status="VALIDATED"); health.record_tick(tick, received_at); store.tick(received_at, tick); lifecycle.update_tick(tick)
        ...
        for candle in one_min.flush(tick.timestamp):
            store.candle(candle); health.candles_1m += 1; health.record_candle(candle)
            context = contexts.get(candle.instrument_id); benchmark_context = next((ctx for ctx in benchmark_contexts.values() if ctx.instrument_id == candle.instrument_id), None)
        ...
        for candle in five_min.flush(tick.timestamp):
            store.candle(candle); health.candles_5m += 1; health.record_candle(candle)
            context = contexts.get(candle.instrument_id); benchmark_context = next((ctx for ctx in benchmark_contexts.values() if ctx.instrument_id == candle.instrument_id), None)
    ...
    for candle in one_min.flush(now):
        store.candle(candle); health.candles_1m += 1; health.record_candle(candle); context = contexts.get(candle.instrument_id); benchmark_context = next((ctx for ctx in benchmark_contexts.values() if ctx.instrument_id == candle.instrument_id), None)
        if context: _safe_context_candle(context, candle); _publish_anatomy(detectors, contexts, benchmark_contexts, candle.instrument_id, settings.data_root)
        if benchmark_context: _safe_context_candle(benchmark_context, candle)
    for candle in five_min.flush(now):
        store.candle(candle); health.candles_5m += 1; health.record_candle(candle); context = contexts.get(candle.instrument_id); benchmark_context = next((ctx for ctx in benchmark_contexts.values() if ctx.instrument_id == candle.instrument_id), None)
        if context: _safe_context_candle(context, candle)
        if benchmark_context: _safe_context_candle(benchmark_context, candle)
        lifecycle.update_candle(candle)
        if context: _emit_v2_signal(detectors, contexts, benchmark_contexts, candle, store, lifecycle, settings.data_root)
