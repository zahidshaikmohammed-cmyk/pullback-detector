...
def _persist_anatomy(data_root: str | Path, instrument_id: int, anatomy: dict) -> None:
    ...
    try:
        tmp.write_text(json.dumps(anatomy, default=str, separators=(",", ":")), encoding="utf-8"); tmp.replace(target)
...
def _publish_anatomy(detectors, contexts, instrument_id: int, data_root: str | Path | None = None) -> None:
    ...
    if detector is not None:
        anatomy=dict(context.snapshot() if context else {}); anatomy.update(detector.last_state or detector.anatomy())
...
async def run_live(settings: Settings, duration_seconds: int = 600) -> dict:
    instruments=InstrumentUniverse.fetch(); store=EventStore(settings.data_root)
    ...
    # Hydrate only from persisted completed candles. No future/incomplete data is introduced.
    for i in instruments:
        ...
        for c in store.recent_candles(i.security_id,300,2500):
            contexts[i.security_id].update_candle(c)
            detectors[i.security_id].history.append(c)
            detectors[i.security_id]._seen_candles.add(c.start)
        _publish_anatomy(detectors,contexts,i.security_id,settings.data_root)
    LIVE_ANATOMY.clear(); deadline=asyncio.get_running_loop().time()+duration_seconds
    async for payload,tick,received_at in client.stream(subscriptions,request_code=17):
        ...
        if source_skew is not None:
            logger.warning("Dhan source clock skew %.1fs for security_id=%s; raw_source_ts=%s normalized_ts=%s receive_ts=%s",source_skew,tick.instrument_id,tick.timestamp.isoformat(),normalized_tick.timestamp.isoformat(),received_at.isoformat());tick=normalized_tick
    ...
    health.reconnects=client.reconnects;report=health.report(now,len(instruments));report["real_dhan_packets_received"]=health.ticks>0;report["verification"]="Dhan official scrip master + Dhan market quote + live WebSocket";report["v2_detector"]=PullbackDetector.LABEL;report["market_context_engine"]="deterministic_v1";report["alerts_enabled"]=False;report["active_setup_count"]=len(lifecycle.active);report["closed_setup_count"]=len(lifecycle.closed)
    market_open=_nse_cash_session_open(now)
    if not market_open:
        report["market_status"]="CLOSED";report["session_state"]="NSE_CASH_SESSION_CLOSED"
        if health.ticks==0:report["dhan_connection_status"]="session_closed";report["no_live_data"]=True;report["last_known_data_retained"]=True
    else:report["market_status"]="OPEN";report["session_state"]="NSE_CASH_SESSION_OPEN"
    store.health(report)
