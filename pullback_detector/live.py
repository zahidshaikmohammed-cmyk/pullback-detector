...
def _publish_anatomy(detectors, contexts, benchmark_contexts, instrument_id: int, data_root: str | Path | None = None) -> None:
    ...
    try:
        ...
        anatomy.update(detector.last_state or detector.anatomy()); anatomy["market_context"] = _equity_context(contexts, benchmark_contexts, instrument_id); anatomy["market_context_version"] = "1.1"; LIVE_ANATOMY[instrument_id] = anatomy
        ...
        LIVE_ANATOMY[instrument_id] = {"instrument_id": instrument_id, "data_freshness": "CALCULATION_ERROR", "context_error": f"{type(exc).__name__}: {exc}"}
...
async def run_live(settings: Settings, duration_seconds: int = 600) -> dict:
    ...
    registry = {i.security_id: i for i in instruments + benchmarks}
    ...
    benchmark_contexts = {i.symbol: MarketContextEngine(i.security_id) for i in benchmarks}; LIVE_ANATOMY.clear()
    ...
    async for payload, tick, received_at in client.stream(subscriptions, request_code=17):
        ...
        if instrument is None:
            health.malformed_packets += 1; logger.warning("TICK_REJECTED reason=UNKNOWN_INSTRUMENT instrument_id=%s", tick.instrument_id); continue
        ...
        # Update first, then finalize completed intervals using the normalized event clock.
        ...
        logger.info("TICK_ACCEPTED instrument_id=%s segment=%s price=%s source_ts=%s normalized_ts=%s receive_ts=%s", tick.instrument_id, tick.exchange_segment, tick.price, (tick.source_timestamp or tick.timestamp).isoformat(), tick.timestamp.isoformat(), received_at.isoformat())

    now = datetime.now(timezone.utc)
    one_min.update if False else None
    ...
    now = datetime.now(timezone.utc); health.reconnects = client.reconnects
